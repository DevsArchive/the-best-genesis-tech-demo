...
endaddr		EQUS	"\eaddr"		; Use eaddr
	...
	move.w	#(((\endaddr)&$FFFFFF)-((\saddr)&$FFFFFF))>>2-1,d1

.Clear\@:
	move.l	d0,(a1)+			; Clear data
	dbf	d1,.Clear\@			; Loop until data is cleared

	if (((\endaddr)&$FFFFFF)-((\saddr)&$FFFFFF))&2
		move.w	d0,(a1)+		; Clear remaining word of data
	endif
	if (((\endaddr)&$FFFFFF)-((\saddr)&$FFFFFF))&1
...
;		  (If left blank, this just uses VDP_CTL instead)
; -------------------------------------------------------------------------
...
.Wait\@:
	if narg>0
		...
		move.w	VDP_CTL,-(sp)	; Get VDP status
	endif
...
dma68k macro &
	...
	if narg>4
		...
		move.l	#$94009300|((((\len)/2)&$FF00)<<8)|(((\len)/2)&$FF),VDP_CTL
		move.l	#$96009500|((((\src)/2)&$FF00)<<8)|(((\src)/2)&$FF),VDP_CTL
		move.w	#$9700|(((\src)>>17)&$7F),VDP_CTL
		vdpCmd	move.w,\dest,\type,DMA,>>16,VDP_CTL
		vdpCmd	move.w,\dest,\type,DMA,&$FFFF,-(sp)
		move.w	(sp)+,VDP_CTL
	endif
...
dmaFill macro &
	...
	if narg>3
		...
		move.l	#$94009300|((((\len)-1)&$FF00)<<8)|(((\len)-1)&$FF),VDP_CTL
		move.w	#$9780,VDP_CTL
		move.l	#$40000080|(((\addr)&$3FFF)<<16)|(((\addr)&$C000)>>14),VDP_CTL
		move.w	#(\byte)<<8,VDP_DAT
		waitDMA
	endif
...
dmaCopy macro &
	...
	if narg>3
		...
		move.l	#$94009300|((((\len)-1)&$FF00)<<8)|(((\len)-1)&$FF),VDP_CTL
		move.l	#$96009500|(((\src)&$FF00)<<8)|((\src)&$FF),VDP_CTL
		move.w	#$97C0,VDP_CTL
		move.l	#$000000C0|(((\dest)&$3FFF)<<16)|(((\dest)&$C000)>>14),VDP_CTL
		waitDMA
	endif
...
ackVSync macro
	...
	addq.w	#1,r_Lag_Cnt.w			; Increment lag frame count
	tst.b	r_VSync.w			; Was the VSync flag set?
	beq.s	.NoVSync\@			; If not, branch
	clr.w	r_Lag_Cnt.w			; Clear lag frame count
	clr.b	r_VSync.w			; Clear VSync flag
	
.NoVSync\@:
	addq.l	#1,r_Frame_Cnt			; Increment frame counter
